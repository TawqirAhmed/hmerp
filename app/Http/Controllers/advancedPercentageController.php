...
class advancedPercentageController extends Controller
{
	public function index(Request $request)
	{
		Cart::destroy();
		$saleId = $request->id;
		$salestoapprove = DB::table('advancedsalestoapprove')->where('id',$saleId)->first();
	 ...
	 //        echo "<pre>";
		...
		return view('inventory.advancedPercentage',compact('costomerid','salestoapprove','paymentDescription'));
	}
   ...
   //      	echo "<pre>";
            ...
            $paymentDescription = json_decode($salestoapprove->payment_description);
            return view('inventory.advancedPercentage',compact('costomerid','salestoapprove','paymentDescription'))->with($notification);
	...
	{
		Cart::destroy();
    	return redirect()->route('ad_salesto');
	}
	...
	public function ApproveSale(Request $request)
	{
		...
		//approve sale ------------------------------------------------------------------------
	        ...
	        $ApproveSale=DB::table('approvedadvancedsales')->insert($dataSales);
	        
	        if ($ApproveSale) {
	        	$dltSalestoapprove = DB::table('advancedsalestoapprove')->where('id',$saleId)->delete();

	        	if ($dltSalestoapprove) {
	        		$notification=array(
		 			'message'=>'Advanced Sales Approved Successfully',
		 			'alert-type'=>'success'
		 		);
	        	Cart::destroy();
		 		return Redirect()->route('ad_salesto');
