...
class ApprovedSaleController extends Controller
{
	public function index(Request $request)
    {
	    ...
	    if ($role == 3) {
	    	if ($request->ajax()) {
		$data = $data = DB::table('approvedsales')
		...
		// return view('inventory.approvedsale');
	...
	{
		$where = array('id' => $id);
		$product = DB::table('approvedsales')->where($where)->first();
	...
	public function updateAS(Request $request)
    {
    	...
    	$product = DB::table('approvedsales')->where('id',$request->id)->first();
    	...
    	$sale = DB::table('approvedsales')->where('id',$request->id)->update($data);
    ...
    public function updateASPayment(Request $request)
    {
    	// echo "<pre>";
    	// print_r($request->all());
    	// exit();
    ...
    public function print(Request $request)
	{
		...
		$information =DB::table('approvedsales')->where('id',$request->id)->first();
	    ...
	    $customer = DB::table('customers')->where('id',$customer_id)->first();
		  ...
		  $invoice->setType("Sale Record");    // Invoice Type
