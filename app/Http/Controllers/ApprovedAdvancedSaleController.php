...
class ApprovedAdvancedSaleController extends Controller
{
	public function index(Request $request)
    {
		...
		return view('inventory.approvedAdvancedSales');
    }

     public function edit($id)
	{
		$where = array('id' => $id);
		$product = DB::table('approvedadvancedsales')->where($where)->first();
	...
	public function updateAS(Request $request)
    {
    	...
    	$product = DB::table('approvedadvancedsales')->where('id',$request->id)->first();
    	...
    	$sale = DB::table('approvedadvancedsales')->where('id',$request->id)->update($data);

    	if ($sale) {
    		$notification=array(
	 			'message'=>'Advanced Sales Update Successfully',
	 			'alert-type'=>'success'
	 		);
	 		return Redirect()->back()->with($notification);
    	}
    ...
    public function print(Request $request)
	{
		...
		$information =DB::table('approvedadvancedsales')->where('id',$request->id)->first();
	    ...
	    $customer = DB::table('customers')->where('id',$customer_id)->first();
		  ...
		  $invoice->setType("Advanced Sale Record");    // Invoice Type
    ...
    //     	$employeeCode = $employee[2];
    	...
    	// $cInfoArray=array();
		  ...
		  $invoice->addTotal("Due Balance",$amountDue);
	...
	{
		$pr = DB::table('approvedadvancedsales')->where('id',$id)->delete();
		return Response::json($product);
	}
    //-----------------------------------------------------------------------------------
